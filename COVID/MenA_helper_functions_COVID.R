...
#_____________________________________________________________________________#
# MJ 5/22/20: Bugfix, allow mini-campaigns to target correct age range.       #
#_____________________________________________________________________________#
...
vaccinate<-function(popslice, vlookup, type, mydate, params) { 
  ...
  if ((type=="campaign" | type=="both") & (month(mydate)==10)) {
    ...
    if (dim(campaign.df)[1] > 0){
      for (i in 1:length(campaign.df$DosesCampaign)){
        cDoses <- campaign.df$DosesCampaign[i]
        if (!is.na(cDoses)){
          age.min <- campaign.df$age_first[i]*12
          age.max <- campaign.df$age_last[i]*12
          eligN.s <- sum(popslice[age.min:age.max, eligibles.s])
          eligN.c <- sum(popslice[age.min:age.max, eligibles.c])
          pcNLH <- ifelse(cDoses <= (eligN.s + eligN.c), cDoses/(eligN.s + eligN.c), 1)
          
          popslice[age.min:age.max, "Vs"] <- 
            popslice[age.min:age.max, "Vs"] + params$ve * pcNLH * rowSums(popslice[age.min:age.max,eligibles.s])
          popslice[age.min:age.max, eligibles.s] <- 
            (1 - params$ve * pcNLH) * popslice[age.min:age.max, eligibles.s]
          popslice[age.min:age.max, "Vc"] <- 
            popslice[age.min:age.max,"Vc"] + params$ve * pcNLH * rowSums(popslice[age.min:age.max,eligibles.c])
          popslice[age.min:age.max, eligibles.c] <-
           (1 - params$ve * pcNLH) * popslice[age.min:age.max, eligibles.c]
