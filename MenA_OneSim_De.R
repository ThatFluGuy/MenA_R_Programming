...
MenASimulationDe <- function(startdt, enddt, fp, initpop, vacc_program, countryparams, region, country, inputdir) { 
  #setup before loop
  ...
  while (theDate <= enddt) {
    ...
    pop[,"De",j] <- deathvec * (pop[,"Ns",j-1] + pop[,"Nc",j-1] + pop[,"Ls",j-1] + pop[,"Lc",j-1] + pop[,"Hs",j-1] + pop[,"Hc",j-1] + 
                                  pop[,"Vs",j-1] + pop[,"Vc",j-1] + pop[,"Di",j-1])
